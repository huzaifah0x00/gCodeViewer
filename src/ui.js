...
GCODE.ui = (function () {
  ...
  const prepareSpeedsInfo = function (layerNum) {
    ...
    for (i = 0; i < layerSpeeds.extrude[z].length; i++) {
      ...
      sortMe[spd] = `<div id='colorBox${
        i
      }' class='colorBox' style='background-color: ${
        GCODE.renderer.getGradientColor(scale)
      }'></div>  = ${
        spd
      }mm/s`;
    }
      ...
      for (i = 0; i < layerSpeeds.retract[z].length; i++) {
        ...
        output.push(
          `<span style='color: ${
            ...
            (parseFloat(layerSpeeds.retract[z][i]) / 60).toFixed(2)
          }mm/s`,
        );
      }
    }
  ...
  const printModelInfo = function () {
    ...
    if (gCodeOptions.volumetricE) {
      const fCrossSection = Math.PI * Math.pow(gCodeOptions.filamentDia / 2.0, 2);
      totalFilament /= fCrossSection;
      totalWeight /= fCrossSection;
      for (const k in filamentByExtruder) filamentByExtruder[k] /= fCrossSection;
    }
