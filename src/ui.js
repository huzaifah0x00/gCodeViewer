...
GCODE.ui = (function () {
  ...
  let sliderVer;
  let sliderHor;
  let gCodeLines = { first: 0, last: 0 };
  let showGCode = false;
  const displayType = { speed: 1, expermm: 2, volpersec: 3 };
  //    var worker;

  const setProgress = function (id, progress) {
    $(`#${id}`)
  ...
  const chooseAccordion = function (id) {
    //        debugger;
    $(`#${id}`).collapse("show");
  };

  const setLinesColor = function (toggle) {
    for (let i = gCodeLines.first; i < gCodeLines.last; i++) {
      if (toggle) {
  ...
  const prepareSpeedsInfo = function (layerNum) {
    const z = GCODE.renderer.getZ(layerNum);
    const layerSpeeds = GCODE.gCodeReader.getModelInfo().speedsByLayer;
    const { max } = GCODE.gCodeReader.getModelInfo();
    const { min } = GCODE.gCodeReader.getModelInfo();
    const renderOptions = GCODE.renderer.getOptions();
    const speedIndex = 0;
    const output = [];
    const sortMe = {};
    let sortKeys = [];
    ...
    for (i = 0; i < layerSpeeds.extrude[z].length; i++) {
      ...
      sortMe[
        spd
      ] = `<div id='colorBox${i}' class='colorBox' style='background-color: ${GCODE.renderer.getGradientColor(
        scale
      )}'></div>  = ${spd}mm/s`;
    }
    ...
    if (typeof layerSpeeds.move[z] !== "undefined") {
      ...
      for (i = 0; i < layerSpeeds.move[z].length; i++) {
        ...
        sortMe[
          spd
        ] = `<div id='colorBox${i}' class='colorBox' style='background-color: ${renderOptions.colorMove}'></div>  = ${spd}mm/s`;
      }
    }
    sortKeys = Object.keys(sortMe).sort((a, b) => b - a);
    for (i = 0; i < sortKeys.length; i++) {
    ...
    if (typeof layerSpeeds.retract[z] !== "undefined") {
      ...
      for (i = 0; i < layerSpeeds.retract[z].length; i++) {
        ...
        output.push(
          `<span style='color: ${
            renderOptions.colorRetract
          }'>&#9679;</span> <span style='color: ${
            renderOptions.colorRestart
          }'>&#9679;</span> = ${(
            parseFloat(layerSpeeds.retract[z][i]) / 60
          ).toFixed(2)}mm/s`
        );
      }
    }
  ...
  const prepareExPerMMInfo = function (layerNum) {
    const z = GCODE.renderer.getZ(layerNum);
    const layerSpeeds = GCODE.gCodeReader.getModelInfo().volSpeedsByLayer;
    const { max } = GCODE.gCodeReader.getModelInfo();
    const { min } = GCODE.gCodeReader.getModelInfo();
    const output = [];
    const sortMe = {};
  ...
  const prepareVolPerSecInfo = function (layerNum) {
    const z = GCODE.renderer.getZ(layerNum);
    const layerSpeeds = GCODE.gCodeReader.getModelInfo().extrusionSpeedsByLayer;
    const { max } = GCODE.gCodeReader.getModelInfo();
    const { min } = GCODE.gCodeReader.getModelInfo();
    const gCodeOptions = GCODE.gCodeReader.getOptions();
    const output = [];
    const sortMe = {};
    let sortKeys = [];
    let i;
  ...
  const printLayerInfo = function (layerNum) {
    const z = GCODE.renderer.getZ(layerNum);
    const segments = GCODE.renderer.getLayerNumSegments(layerNum);
    const renderOptions = GCODE.renderer.getOptions();
    const filament = GCODE.gCodeReader.getLayerFilament(z);
    let output = [];

    let aggFilamentUsed = 0.0;
    for (let x = 0; x <= layerNum; x++) {
      layerFilament = GCODE.gCodeReader.getLayerFilament(
        GCODE.renderer.getZ(x)
      );
      if (layerFilament) {
    ...
    output.push(
      `Print time for layer: ${parseFloat(
        GCODE.gCodeReader.getModelInfo().printTimeByLayer[z]
      ).toFixed(1)}sec`
    );
  ...
  const printModelInfo = function () {
    const resultSet = [];
    const modelInfo = GCODE.gCodeReader.getModelInfo();
    const gCodeOptions = GCODE.gCodeReader.getOptions();

    let { totalFilament } = modelInfo;
    let { totalWeight } = modelInfo;
    const { filamentByExtruder } = modelInfo;
    if (gCodeOptions.volumetricE) {
      const fCrossSection =
        Math.PI * Math.pow(gCodeOptions.filamentDia / 2.0, 2);
      totalFilament /= fCrossSection;
      totalWeight /= fCrossSection;
      for (const k in filamentByExtruder)
        filamentByExtruder[k] /= fCrossSection;
    }

    resultSet.push(
      `Model size is: ${modelInfo.modelSize.x.toFixed(
        2
      )}x${modelInfo.modelSize.y.toFixed(2)}x${modelInfo.modelSize.z.toFixed(
        2
      )}mm<br>`
    );
    resultSet.push(`Total filament used: ${totalFilament.toFixed(2)}mm<br>`);
    resultSet.push(
      `Total filament weight used: ${totalWeight.toFixed(2)}grams<br>`
    );
    let i = 0;
    const tmp = [];
    for (const key in modelInfo.filamentByExtruder) {
      i++;
      tmp.push(
        `Filament for extruder '${key}': ${filamentByExtruder[key].toFixed(
          2
        )}mm<br>`
      );
    }
    ...
    resultSet.push(
      `Estimated print time: ${parseInt(
        parseFloat(modelInfo.printTime) / 60 / 60
      )}:${parseInt((parseFloat(modelInfo.printTime) / 60) % 60)}:${parseInt(
        parseFloat(modelInfo.printTime) % 60
      )}<br>`
    );
    resultSet.push(
      `Estimated layer height: ${modelInfo.layerHeight.toFixed(2)}mm<br>`
    );
    resultSet.push(
      `Layer count: ${modelInfo.layerCnt.toFixed(
        0
      )}printed, ${modelInfo.layerTotal.toFixed(0)}visited<br>`
    );
    resultSet.push(
      `Time cost: ${(
        (modelInfo.printTime * gCodeOptions.hourlyCost) /
        60 /
        60
      ).toFixed(2)}<br>`
    );
    resultSet.push(
      `Filament cost: ${(totalWeight * gCodeOptions.filamentPrice).toFixed(
        2
      )}<br>`
    );
  ...
  const handleFileSelect = function (evt) {
    ...
    for (var i = 0, f; (f = files[i]); i++) {
      if (f.name.toLowerCase().match(/^.*\.(?:gcode|g|txt|gco|gx)$/)) {
        ...
        output.push(
          "<li><strong>You should only upload *.gcode files! I will not work with this one!</strong></li>"
        );
        document.getElementById("errorList").innerHTML = `<ul>${output.join(
          ""
        )}</ul>`;
        return;
      }
      ...
      reader.onload = function (theFile) {
        ...
        if (showGCode) {
          ...
          myCodeMirror.setValue(
            "GCode view is disabled. You can enable it in 'GCode analyzer options' section."
          );
        }
      };
      reader.readAsText(f);
  ...
  const initSliders = function () {
    ...
    const onLayerChange = function (val) {
      const progress = GCODE.renderer.getLayerNumSegments(val) - 1;
      ...
      gCodeLines = GCODE.gCodeReader.getGCodeLines(
        val,
        sliderHor.slider("values", 0),
        sliderHor.slider("values", 1)
      );
  ...
  const checkCapabilities = function () {
    const warnings = [];
    const fatal = [];
    ...
    if (!Modernizr.canvas)
      fatal.push(
        "<li>Your browser doesn't seem to support HTML5 Canvas, this application won't work without it.</li>"
      );
    if (!Modernizr.filereader)
      fatal.push(
        "<li>Your browser doesn't seem to support HTML5 File API, this application won't work without it.</li>"
      );
    ...
    if (!Modernizr.svg)
      fatal.push(
        "<li>Your browser doesn't seem to support HTML5 SVG, this application won't work without it.</li>"
      );

    if (fatal.length > 0) {
      document.getElementById("errorList").innerHTML = `<ul>${fatal.join(
        ""
      )}</ul>`;
    ...
    if (!Modernizr.draganddrop)
      warnings.push(
        "<li>Your browser doesn't seem to support HTML5 Drag'n'Drop, Drop area will not work.</li>"
      );

    if (warnings.length > 0) {
      document.getElementById("errorList").innerHTML = `<ul>${warnings.join(
        ""
      )}</ul>`;
  ...
  return {
    worker: undefined,
    initHandlers() {
      const capabilitiesResult = checkCapabilities();
    ...
    processOptions() {
      GCODE.gCodeReader.setOption({
        ...
        purgeEmptyLayers: document.getElementById("purgeEmptyLayersCheckbox")
          .checked,
        ...
        filamentType: document.getElementById("plasticABS").checked
          ? "ABS"
          : "PLA",
      });
      ...
      GCODE.renderer.setOption({
        ...
        differentiateColors: document.getElementById(
          "differentiateColorsCheckbox"
        ).checked,
